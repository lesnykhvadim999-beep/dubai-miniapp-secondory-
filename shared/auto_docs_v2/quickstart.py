"""PHASE BO O4 — quickstart.

Writes a hardcoded reference guide to C:/Projects/shared/docs/QUICK_START.md.
Content is NOT LLM-generated (deliberate — must be reliable).
No DSN / token / password values ever included.
"""
...
from .schema import upsert_doc_inventory

OUTPUT_PATH = Path(r"C:\Projects\shared\docs\QUICK_START.md")


BODY = """# Vadim Realty Ecosystem — Quick Start
...
def generate() -> Path:
    ...
    body = BODY.format(ts=datetime.utcnow().strftime("%Y-%m-%d %H:%M UTC"))
    OUTPUT_PATH.write_text(body, encoding="utf-8")
    try:
        upsert_doc_inventory(str(OUTPUT_PATH), "quick_start", len(body.encode("utf-8")))
    except Exception:
        pass
    return OUTPUT_PATH
...
if __name__ == "__main__":
    p = generate()
    print(f"[quickstart] wrote {p} ({p.stat().st_size} bytes)")
